...
def load_sarar(img_dir, ann_dir=None, classes=None, nproc=10):
    classes = get_classes('SARAR' if classes is None else classes)
    cls2lbl = {cls: i for i, cls in enumerate(classes)}

    print('Starting loading SARAR dataset information.')
    start_time = time.time()
    _load_func = partial(_load_sarar_single,
                         img_dir=img_dir,
                         ann_dir=ann_dir,
                         cls2lbl=cls2lbl)
    if nproc > 1:
    ...
    end_time = time.time()
    print(f'Finishing loading SARAR, get {len(contents)} images,',
          f'using {end_time-start_time:.3f}s.')

    return contents, classes


def _load_sarar_single(imgfile, img_dir, ann_dir, cls2lbl):
    ...
    content = _load_sarar_txt(txtfile, cls2lbl)

    content.update(dict(width=size[0], height=size[1], filename=imgfile, id=img_id))
    return content


def _load_sarar_txt(txtfile, cls2lbl):
...
def load_sarar_submission(ann_dir, img_dir=None, classes=None, nproc=10):
    classes = get_classes('SARAR' if classes is None else classes)
    ...
    print('Starting loading SARAR submission information')
    start_time = time.time()
    infos_per_cls = []
    for cls in classes:
        if cls not in cls2file_mapper:
            ...
            infos_per_cls.append(_load_sarar_submission_txt(subfile))
    ...
    end_time = time.time()
    print(f'Finishing loading SARAR submission, get{len(contents)} images,',
          f'using {end_time-start_time:.3f}s.')
    return contents, classes


def _load_sarar_submission_txt(subfile):
...
def save_sarar_submission(save_dir, id_list, dets_list, task='Task1', classes=None, with_zipfile=True):
    assert task in ['Task1', 'Task2']
    classes = get_classes('SARAR' if classes is None else classes)

    # if osp.exists(save_dir):
    #     num = 0
    #     while osp.exists(save_dir+f'_{num}'):
    #         num+=1
    #     print(f'The save_dir should be a non-exist path, but {save_dir} is existing, save to {save_dir+f"_{num}"}')
    #     save_dir+=f'_{num}'
...
def save_xml(img_id,pred,out_folder):
    ...
    str+='        <pluginname>Airplane Detection and Recognition</pluginname>\n'
    str+='        <pluginclass>Detection</pluginclass>\n'
    str+='        <time>2021-07-2021-11</time>\n'
    str+='    </research>\n'
    str+='    <objects>\n'
    for v in pred:
        category = v["category"]
