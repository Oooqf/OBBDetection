...
data_root = '/root/datasets/split_ms_fair1m/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type='FAIR1MDataset',
        task='Task1',
        ann_file='/root/datasets/split_ms_fair1m/train/annfiles/',
        img_prefix='/root/datasets/split_ms_fair1m/train/images/',
        pipeline=train_pipeline
    ),
    val=dict(
        type='FAIR1MDataset',
        task='Task1',
        ann_file='/root/datasets/split_ms_fair1m/val/annfiles/',
        img_prefix='/root/datasets/split_ms_fair1m/val/images/',
        pipeline=test_pipeline),
    test=dict(
        type='FAIR1MDataset',
        task='Task1',
        ann_file='/root/datasets/split_ms_fair1m/test/annfiles/',
        img_prefix='/root/datasets/split_ms_fair1m/test/images/',
        pipeline=test_pipeline))
...
log_level = 'INFO'
load_from = None
resume_from = r'work_dirs/faster_rcnn_r50_fpn_1x_fair1m_cal3/epoch_12.pth'
workflow = [('train', 1)]
model = dict(
    type='OrientedRCNN',
...
test_cfg = dict(
    ...
    rcnn=dict(
        ...
        max_per_img=2000))
work_dir = './work_dirs/faster_rcnn_r50_fpn_1x_fair1m_cal3'
gpu_ids = range(0, 1)
