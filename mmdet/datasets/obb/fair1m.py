...
import torch
from mmdet.core.bbox.transforms_obb import arb2result
from .wbf import *
...
class FAIR1MDataset(CustomDataset):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.task = 'Task1'
        super(FAIR1MDataset, self).__init__(*args, **kwargs)
    ...
    def load_annotations(self, ann_file):
        ...
        contents.sort(key=lambda x:x['filename'])
    ...
    def format_results(self,
        ...
        if not with_merge:
            ...
            if save_dir is not None:
                ...
                bt.save_fair1m_submission(save_dir, id_list, dets_list, task, self.CLASSES)
            return results
        ...
        merged_results = [merge_func(i) for i in tqdm(collector.items())]
        # merged_results = mmcv.track_parallel_progress(merge_func, list(collector.items()), nproc)
        if use_wbf :
            for pic_index in tqdm(range(len(merged_results))):
                merged_results[pic_index] = list(merged_results[pic_index])
                rbbox = merged_results[pic_index]
                det_bboxes_sum = torch.tensor([[], []])
                det_bboxes_sum = torch.reshape(det_bboxes_sum, [0, 0])
                det_labels_sum = torch.tensor([])
                det_labels_sum = torch.reshape(det_labels_sum, [0, ])
                flag = True
                for label in range(len(rbbox[1])):
                    np_rbbox_list = rbbox[1]
                    if(len(np_rbbox_list[label])!=0):
                        det_bboxes = torch.from_numpy(np_rbbox_list[label])
                        det_labels = torch.tensor([label]*len(det_bboxes))
                        det_labels = torch.reshape(det_labels, [len(det_bboxes), ])
                        if flag:
                            det_bboxes_sum = det_bboxes
                            det_labels_sum = det_labels
                            flag = False
                        else:
                            det_bboxes_sum = torch.cat((det_bboxes_sum, det_bboxes), dim=0)
                            det_labels_sum = torch.cat((det_labels_sum, det_labels), dim=0)
                if len(det_bboxes_sum) > 0:
                    scores_list = det_bboxes_sum[:, -1]
                    boxes_list = det_bboxes_sum[:, :-1]
                    det_bboxes_sum, det_labels_sum = weighted_boxes_fusion(boxes_list, scores_list, det_labels_sum, iou_thr=0.44)
                merged_results[pic_index][1] = arb2result(det_bboxes_sum, det_labels_sum, len(self.CLASSES),
                                                                            bbox_type='obb')
                merged_results[pic_index] = tuple(merged_results[pic_index])
    ...
    def format_results_single(self,
                       start_id,
                       results,
                       with_merge=True,
                       ign_scale_ranges=None,
        ...
        collector = defaultdict(list)
        infos = self.data_infos[start_id:start_id+len(results)]
        for data_info, result in zip(infos, dets):
            if ign_scale_ranges is not None:
                img_scale = data_info['width']
                scale_ratio = np.array(split_sizes) / img_scale
                inds = np.argmin(abs(np.log(scale_ratio)))
        ...
        merge_func = partial(_merge_func, CLASSES=self.CLASSES, iou_thr=iou_thr, task=task)
        merged_results = [merge_func(i) for i in collector.items()]
        # merged_results = mmcv.track_parallel_progress(merge_func, list(collector.items()), nproc)

        for pic_index in range(len(merged_results)):
            merged_results[pic_index] = list(merged_results[pic_index])
            rbbox = merged_results[pic_index]
            det_bboxes_sum = torch.tensor([[], []])
            det_bboxes_sum = torch.reshape(det_bboxes_sum, [0, 0])
