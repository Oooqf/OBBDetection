...
class SAC(nn.Module):
    def __init__(self,feat_channels,num_classes):
        ...
        self.pix_attn_sac_out = ConvModule(
                self.feat_channels,
                self.num_classes,
                3,stride=1,padding=1,
                norm_cfg=dict(type='BN', requires_grad=True),
                inplace=False)
        self.pix_attn_out = ConvModule(
                self.feat_channels,
                1,
                3,stride=1,padding=1,
                norm_cfg=dict(type='BN', requires_grad=True),
    ...
    def forward(self,features):
        ...
        pix_attn_sac = self.pix_attn_sac_out(pix_attn)
        pix_attn = self.pix_attn_out(pix_attn)
...
class SACRPNHead(RPNTestMixin, OBBAnchorHead):
    ...
    def __init__(self, in_channels,sac_classes,img_size = 1024, **kwargs):
        self.sac_classes = sac_classes + 1
    ...
    def _init_layers(self):
        ...
        self.sac = SAC(self.feat_channels,self.sac_classes)
    ...
    def forward_train(self,
        ...
        if proposal_cfg is None:
            ...
            proposal_list = self.get_bboxes(*outs[:2], img_metas, cfg=proposal_cfg)
    ...
    def loss(self,
        ...
        for level in range(len(pixel_attns)): #level
            mask = np.zeros((batch_size,self.sac_classes,self.img_size,self.img_size,1))
            for i in range(batch_size): #batch
                map_levels = map_obb_levels(gt_bboxes[i],len(pixel_attns))
                index = ((map_levels-level).abs()<=1)
                if index.numel()<=0:
                    continue
                polys = obb2poly(gt_bboxes[i][index]).cpu().numpy().astype(np.int).reshape((-1,4,2))
                for poly, label in zip(polys,gt_labels[i][index]):
                    cv2.fillPoly(mask[i][label],[poly],1.0)
            mask = torch.tensor(mask.reshape(batch_size,self.sac_classes,self.img_size,self.img_size),dtype=torch.float)
            mask = F.interpolate(mask,pixel_attns[level].shape[2:])
            mask = mask.to(cls_scores[0].device)
            masks.append(mask)
        loss_sac = [self.sac.loss(pixel_attn,mask) for pixel_attn,mask in zip(pixel_attns,masks)]
        return dict(
