...
def partition(x, size):
    ...
    return x.permute(0, 1, 2, 4, 3, 5).reshape(B, C, H // size * W // size, size, size).contiguous()

@HEADS.register_module()
class OBBRextractRoIHead(OBBBaseRoIHead, OBBoxTestMixin):
    """Simplest base roi head including one bbox head and one mask head.
    """
    def __init__(self, bbox_roi_extractor=None, bbox_roi_rextractor=None, bbox_head=None, shared_head=None, refine_roi_num=4, train_cfg=None, test_cfg=None):
        self.extract_size = bbox_roi_extractor['roi_layer']['out_size']
        self.rextract_size = bbox_roi_rextractor['roi_layer']['out_size']
        self.refine_roi_num = refine_roi_num
        ...
        self.bbox_roi_rextractor = build_roi_extractor(bbox_roi_rextractor)
    ...
    def init_weights(self, pretrained):
        ...
        if self.with_bbox:
            ...
            self.bbox_roi_rextractor.init_weights()
    ...
    def forward_train(self,
        ...
        return losses
    def get_peak_roi_feats(self,feats,raw_feats):
        
        map_size = self.rextract_size // self.extract_size
        B,C,H,W = feats.shape
        sum_feats = feats.sum(dim=1)
        _,ids = sum_feats.reshape(B,H*W).topk(self.refine_roi_num)
        raw_feats = partition(raw_feats,map_size)
        t = []
        for i in range(B):
            t.append(raw_feats[None,i,:,ids[i]])
            print(t[i].shape)
    ...
    def _bbox_forward(self, x, rois):
        ...
        rexact_bbox_feats = self.bbox_roi_rextractor(
            x[:self.bbox_roi_extractor.num_inputs], rois)
        if self.with_shared_head:
            bbox_feats = self.shared_head(bbox_feats)
        results = self.bbox_head(bbox_feats)

        peak_roi_feats = self.get_peak_roi_feats(results['cls_feats'],rexact_bbox_feats)

        bbox_results = dict(
            cls_score=results['cls_score'], bbox_pred=results['bbox_pred'], bbox_feats=bbox_feats)
        return bbox_results
