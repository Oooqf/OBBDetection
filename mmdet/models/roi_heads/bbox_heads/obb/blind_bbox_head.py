...
class BlindBoxHead(OBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(BlindBoxHead, self).__init__(*args, **kwargs)
        ...
        self.blind_dropout = torch.nn.Dropout2d(p=dropout_prob)
    def _add_conv_fc_branch(self,
        ...
        if num_branch_fcs > 0:
            ...
            fc_out_channels = self.cls_fc_out_channels if is_cls else self.reg_fc_out_channels
    ...
    def init_weights(self):
        super(BlindBoxHead, self).init_weights()
    ...
    def blind_drop(self,input):
        input = input.permute((2,3,0,1))
        output = self.blind_dropout(input)
    ...
    def forward(self, x):
        ...
        # x_cls = torch.cat([x_cls,x_reg],dim=1)
...
@HEADS.register_module()
class FGBBoxHead(BlindBoxHead):
    def __init__(self, *args, **kwargs):
        super(FGBBoxHead, self).__init__(
            num_shared_convs=1,
            num_shared_fcs=0,
            num_cls_convs=2,
            num_cls_fcs=2,
