...
#         loss = self.weight * (1.0 - loss.sum() / (features.shape[0]-1) / features.shape[0])
#         return loss
...
class AlienateModule(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(self.parallel_num):
            attn_map = x[:,i].sum(dim=1).sigmoid()[:,None,None,:,:]
        ...
        # loss_similarity = self.loss_similarity(attn_maps.permute(1,0,2,3,4).reshape(self.parallel_num,-1))
