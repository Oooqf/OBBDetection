...
#     def __init__(self,drop_threshold=0.7):
#         '''Args:
#             tempearture: a constant to be divided by consine similarity to enlarge the magnitude
#             iou_threshold: consider proposals with higher credibility to increase consistency.
#         '''
...
#         avg_features = torch.sigmoid(features.mean(dim=1)).reshape(B,1,H,W)
#         mask = torch.ones((B,1,H,W),device=features.device)
#         mask[avg_features>self.drop_threshold]=0.0
...
#         index = sum_features.reshape(B,H*W).argmax(dim=1)
#         mask_pos_x = index // H - (self.mask_size//2)
#         mask_pos_y = index % H - (self.mask_size//2)
#         mask_pos_x = torch.clip(mask_pos_x,0,H-self.mask_size-1)
#         mask_pos_y = torch.clip(mask_pos_y,0,W-self.mask_size-1)
#         mask = torch.zeros_like(features,device=features.device).to(torch.bool)
#         for b in range(B):
#             mask[b,:,mask_pos_x[b]:mask_pos_x[b]+self.mask_size, mask_pos_y[b]:mask_pos_y[b]+self.mask_size] = True
#         x = features.masked_fill(mask,value = 0.0)
#         return x
...
#         B,C,H,W = features.shape
#         sum_features = features.sum(dim=1).reshape(B,H*W)
#         _,index = sum_features.topk(self.mask_size,dim=1,sorted=False)
#         mask = torch.ones_like(sum_features,device=features.device)
#         mask.scatter_(dim=1,index=index,value=0.0)
#         return features * mask.reshape(B,1,H,W)

@HEADS.register_module()
class InpaintBoxHead(OBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(InpaintBoxHead, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(InpaintBoxHead, self).init_weights()
