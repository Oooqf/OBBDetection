...
def obb2poly(obboxes):
    ...
    return torch.cat(
        [point1, point2, point3, point4], dim=-1).cpu().numpy().astype(np.int32)
...
def show_bbox(image, bboxs_list, color=None,
    ...
    for bbox in bboxs_list:
        ...
        bbox_f = np.array(bbox[:5], np.int32)
        if color is None:
...
def show_obbox(image, bboxs_list, labels=None, color=None,
              thickness=1, font_scale=0.3, wait_time_ms=0, names=None,
              is_show=True):
    """
    Visualize bbox in object detection by drawing rectangle.
    ...
    :param image: numpy.ndarray.
    :param obboxs_list: list: [pts_xyxy,theta, prob]: prediction.
    :param label: list: label list.
    :param color: tuple.
    :param thickness: int.
    :param fontScale: float.
    :param wait_time_ms: int
    ...
    for i,bbox in enumerate(bboxs_list):
        txt = ''
        if len(bbox) == 6:
            txt = '{:.3f}'.format(bbox[5])
        if labels is not None:
            txt += f'id={labels[i]}'
        bbox_f = obb2poly(bbox[:5]).reshape(4,2)
        if color is None and labels is None:
            color = random_color(rgb=True).astype(np.float64)
        cv2.polylines(image_copy, [bbox_f], 1, color, 1)
        if txt:
            cv2.putText(image_copy, txt, bbox_f[0],
                        font, font_scale, (255, 255, 255), thickness=thickness, lineType=cv2.LINE_AA)
    if is_show:
        show_img(image_copy, names, wait_time_ms)
    return image_copy
...
def show_tensor(tensor, resize_hw=None, top_k=50, mode='CHW', is_show=True,
                wait_time_ms=0, show_split=True, is_merge=True, row_col_num=(1, -1), combine_method='sum'):
    """
